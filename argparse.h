...
template <typename Type> class TraitsProvider {
public:
...
inline std::tuple<std::string, std::optional<std::string>>
SplitLongArg(const std::string &arg) {
  auto pos = arg.find("=");
...
bool IsValidValue(const Type &value, const std::vector<Type> &options) {
  return std::any_of(
      ...
      [&value](const Type &option) { return Equal(value, option); });
}

} // namespace detail

template <> class TypeTraits<std::string> {
public:
...
template <> class TypeTraits<long long int> {
public:
...
template <> class TypeTraits<unsigned long long int> {
public:
...
#define ARGPARSE_DEFINE_TRAITS(Type, BaseType)                                 \
  template <> class TypeTraits<Type> {                                         \
  public:                                                                      \
    static Type FromString(const std::string &str) {                           \
...
public:
  ArgHolderBase(std::string fullname, char shortname, std::string help)
      : fullname_(std::move(fullname)), shortname_(shortname),
        help_(std::move(help)), required_(false) {}
  ...
  virtual const std::string &fullname() const { return fullname_; }
...
public:
  FlagHolder(std::string fullname, char shortname, std::string help)
      : ArgHolderBase(fullname, shortname, help), value_(0) {}

  virtual bool HasValue() const override { return true; }

  virtual bool RequiresValue() const override { return false; }

  virtual void ProcessFlag() override { value_++; }
...
template <typename Type> class ValueHolderBase : public ArgHolderBase {
public:
  using ArgHolderBase::ArgHolderBase;

  virtual bool RequiresValue() const final { return true; }
...
template <typename Type> class ValueHolder : public ValueHolderBase<Type> {
public:
...
template <typename Type> class MultiValueHolder : public ValueHolderBase<Type> {
public:
  using ValueHolderBase<Type>::ValueHolderBase;
...
public:
  FlagHolderWrapper(FlagHolder *ptr) : ptr_(ptr) {}

  size_t operator*() const { return ptr_->value(); }

private:
  FlagHolder *ptr_;
};

template <typename Type> class ArgHolderWrapper {
public:
  ArgHolderWrapper(ValueHolder<Type> *ptr) : ptr_(ptr) {}
  ...
  explicit operator bool() const { return ptr_->HasValue(); }

  const Type &operator*() const { return ptr_->value(); }
...
template <typename Type> class MultiArgHolderWrapper {
public:
  MultiArgHolderWrapper(MultiValueHolder<Type> *ptr) : ptr_(ptr) {}
  ...
  explicit operator bool() const { return !ptr_->values().empty(); }

  const std::vector<Type> &operator*() const { return ptr_->values(); }
...
public:
  ...
  template <typename Type>
  MultiArgHolderWrapper<Type>
  AddMultiArg(const std::string &fullname, char shortname,
              const std::string &help = kDefaultHelpString) {
    CheckOptionEntry(fullname, shortname);
    UpdateShortLongMapping(fullname, shortname);
    auto holder =
...
inline FlagHolderWrapper
AddGlobalFlag(const std::string &fullname, char shortname,
              const std::string &help = kDefaultHelpString) {
  return GlobalHolders()->AddFlag(fullname, shortname, help);
}

inline FlagHolderWrapper
AddGlobalFlag(const std::string &fullname,
              const std::string &help = kDefaultHelpString) {
  return AddGlobalFlag(fullname, '\0', help);
}

template <typename Type>
ArgHolderWrapper<Type>
AddGlobalArg(const std::string &fullname, char shortname,
             const std::string &help = kDefaultHelpString) {
...
template <typename Type>
ArgHolderWrapper<Type>
AddGlobalArg(const std::string &fullname,
             const std::string &help = kDefaultHelpString) {
...
template <typename Type>
MultiArgHolderWrapper<Type>
AddGlobalMultiArg(const std::string &fullname, char shortname,
                  const std::string &help = kDefaultHelpString) {
...
template <typename Type>
MultiArgHolderWrapper<Type>
AddGlobalMultiArg(const std::string &fullname,
                  const std::string &help = kDefaultHelpString) {
...
public:
  Parser()
      : holders_(), positionals_(), free_args_(std::nullopt),
        parse_global_args_(true), usage_string_(std::nullopt),
  ...
  template <typename Type>
  MultiArgHolderWrapper<Type>
  AddMultiArg(const std::string &fullname,
              const std::string &help = kDefaultHelpString) {
    return AddMultiArg<Type>(fullname, '\0', help);
  }

  template <typename Type> ArgHolderWrapper<Type> AddPositionalArg() {
  ...
  void EnableFreeArgs() { free_args_ = std::vector<std::string>(); }

  void IgnoreGlobalFlags() { parse_global_args_ = false; }
  ...
  const std::vector<std::string> &FreeArgs() const { return *free_args_; }
